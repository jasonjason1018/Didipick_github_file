...
function in_cart(src,store,item,links,flag){
	if(links == 0){
		...
		var tprice = document.buyAction.tw_price.value;
		if(spec != ""){
			// getData('../incarts.php?src='+src+'&store='+encodeURIComponent(store)+'&item='+encodeURIComponent(item)+'&spec='+encodeURIComponent(spec)+'&num='+encodeURIComponent(num)+'&jprice='+encodeURIComponent(jprice)+'&tprice='+encodeURIComponent(tprice)+'&links='+encodeURIComponent(links)+'&flag='+encodeURIComponent(flag),'cartNum');
			$.post("../incarts.php",{
				src: src,
				store: store,
				item: item,
