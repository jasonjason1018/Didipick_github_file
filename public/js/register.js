    ...
    XMLHttpRequestObject.onreadystatechange = function()
    {
          ...
          XMLHttpRequestObject.status == 200)
      {
		...
		if(xml == "true"){
			...
			document.getElementById("sub1").className = "btn type_fill z-regular js-call_spec";
...
function checklogin(){
	...
	if(authcode.length >= 6 && mobi.length == 10 && re.test(mobi) == true){
		...
		document.getElementById("sub2").className = "btn type_fill z-regular";
	}else{
		document.login.sub2.disabled = true;
		document.getElementById("sub2").className = "btn type_fill z-second";
	}
}
/* function TEST(){
	var name = document.reg_data.realname.value;
	var passport = document.reg_data.passport.value;
	...
	getData('http://tokukai.com/testcode.php?name='+encodeURIComponent(name)+'&passport='+encodeURIComponent(passport)+'&mobile='+encodeURIComponent(mobi)+'&auth='+encodeURIComponent(auth) + '&email=' + encodeURIComponent(email) + '&passwd=' + encodeURIComponent(passwd) + '&invite=' + encodeURIComponent(invite) + '&chkMail=' + encodeURIComponent(chkMail),'reg_final');
...
function checkPass2(control) {
	...
	if (control.value == "" || re.test(control.value) == false || control.value.length < 6 || control.value.length > 20) {
		...
		document.getElementById("subb").className = "btn type_fill z-second";
		return (false);
	}
	return (true);
}
...
function validateForm(form){
	...
	if (!checkID(form.passport.value)) return;
	...
	document.getElementById("subb").className = "btn type_fill z-regular js-call_spec";
	return(true);
}
function logout(){
	location = "https://tokukai.com/logout.php";
}
// function follow(id){
	// gatData('https://tokukai.com/follow.php?id='+id,'noreturn');
// }
// function compare(id){
	// gatData('https://tokukai.com/compare.php?id='+id,'noreturn');
// }
...
function checkEmail3(email){
	...
	if (email.length==0) {
		...
		document.getElementById("forgetSubmit").className = "btn type_fill z-second js-call_spec";
		return (false);
	} else if (index==-1) {
		// alert("錯誤的E-mail格式。");
		document.forgetForm.forgetS.disabled = true;
		...
		document.getElementById("forgetSubmit").className = "btn type_fill z-regular";
		return (true);
	}
}