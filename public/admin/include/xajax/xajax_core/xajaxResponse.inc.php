...
class xajaxResponse
{
	...
	*/
	 ...
	 * 依據 XML 建立對應下拉選項
	 * 
	 * <i>使用方法:</i> <kbd>$objResponse->addOption("物件ID", "選項值", "選項文字內容", "是否選取此選項");</kbd>
	 * 
	 * @param string $idName 物件ID
	 * @param string $optionVal 選項值
	 * @param string $optionText 選項文字內容
	 * @param boolean $is_selected 是否選取此選項，0 = 否，1 = 是
	 * @return object
	 */
	function addOption($idName, $optionVal, $optionText, $is_selected = 0)
	{
		$select_status = ($is_selected) ? "true" : "false";
		$script = "var oOption = document.createElement('OPTION');
				  " .$idName. ".options.add(oOption);
				  oOption.innerHTML = '" .$optionText. "';
				  oOption.value = '" .$optionVal. "';
				  oOption.selected = " .$select_status;
		$this->script($script);
		return $this;
	}	
