...
function Page2($page,$perpage,$result,$site,$search,$key_word,$during,$date_to,$date_from){
	if($during){ $during_k = "during=".$during."&"; }
	if($date_to){ $date_to_k = "date_to=".$date_to."&"; }
	if($date_from){ $date_from_k = "date_from=".$date_from."&";}
	$page = intval($page);
...
//	echo "<p align=\"center\"><font size=\"2\"></font><font>";
	$up = $page-1;
	if($page > 1){ 
		echo "<li></font><font size=\"2\"><a href='".$site."?page=1&".$during_k."key=".$key_word."&".$date_to_k."search=".$search."&".$date_from_k."'>".Page_00."</a>&nbsp;</font></li>";
		echo "<li><font size=2><a href='".$site."?page=".$up."&".$during_k."key=".$key_word."&".$date_to_k."search=".$search."&".$date_from_k."'>".Page_01."</a>&nbsp;|&nbsp;</font></li>"; 
	}
	for($i=$start;$i<=$end;$i++){

		if($i == $page){ echo "<li class=\"selected\">"; }else{ echo "<li>";}
		echo "<a href='".$site."?page=".$i."&".$during_k."key=".$key_word."&".$date_to_k."search=".$search."&".$date_from_k."'>".$i."</a>&nbsp&nbsp"; 
		if($i == $page){ echo "</li>"; }else{ echo "</li>";}

	}
	$down = $page+1;
	if($down <= $total_page_num){ echo "<li><font size=2>&nbsp|&nbsp;<a href='".$site."?page=".$down."&".$during_k."key=".$key_word."&".$date_to_k."search=".$search."&".$date_from_k."'>".Page_02."</a></font></li>"; }
	$total_p = intval($total_page_num);
//	echo "</font>";
	if($total_p > 1){
		echo "<li><font size=\"2\">&nbsp;<a href='".$site."?page=".$total_p."&".$during_k."key=".$key_word."&".$date_to_k."search=".$search."&".$date_from_k."'>".Page_03."</a></font></li>";
	}
//	echo "</p>";	
}
...
function Yahoo_shList($cate = 1,$page = 0,$sort = 1,$query,$store_id,$type,$image_size = 132,$prod_id,$pers_id,$brand_id,$price_from,$price_to,$condition = 'all')
{
	...
	$toURL.= "?appid=".$Y_s_appid."&category_id=".$cate."&hits=".$hits."&offset=".($page*$hits);
	if($sort != "") $toURL.= "&sort=".$sort_th;
	...
	if($store_id != "") $toURL.= "&store_id=".$store_id;
