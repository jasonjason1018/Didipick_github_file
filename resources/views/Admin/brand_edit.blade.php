...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1">
  ...
  <tr>-->
    <tr>
    <td width="100%">
    	<form method="POST" action="/brand_edit_update" name="Links" enctype="multipart/form-data" id="product_edit_form">
    		@csrf
    <table border="1" cellpediting="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1">
    	@foreach($data as $v)
		  ...
		  <tr>
		    <td width="18%" bgcolor="#C0C0C0" align="center"><font size="2">品牌Logo</font></td>
		    <td width="82%">
		    	@if($v->icon == '')
		    		<input type="file" size="42" name="pic1" id="pic1" value="">
		    	@else
		    		@php
			    		$img = explode('/', $v->icon);
			    		$img = $img[1];
			    	@endphp
		    		<input type="text" name="pic1" value="{{$v->icon}}">
		    		<button><a href="/brand_img/{{$img}}" target="_blank">圖片</a></button>
		    		<button><a href="/brand_delete_edit_img/{{$img}}/{{$v->sno}}/1">刪除</a></button>
		    	@endif
		    </td>
		  </tr>
		  <tr>
		    <td width="18%" bgcolor="#C0C0C0" align="center"><font size="2">品牌大圖</font></td>
		    <td width="82%">
		    	@if($v->icon2 == '')
		    		<input type="file" size="42" name="pic2" id="pic2" value="">
		    	@else
		    		@php
			    		$img = explode('/', $v->icon2);
			    		$img = $img[1];
			    	@endphp
		    		<input type="text" name="pic2" value="{{$v->icon2}}">
		    		<button><a href="/brand_img/{{$img}}" target="_blank">圖片</a></button>
		    		<button><a href="/brand_delete_edit_img/{{$img}}/{{$v->sno}}/2">刪除</a></button>
		    	@endif
		    </td>
		  </tr>
		  <tr>
		    <td width="18%" bgcolor="#C0C0C0" align="center"><font size="2">瀏覽數</font></td>
		    <td width="82%"><input type="text" size="42" name="view" id="view" value="{{$v->view}}"></td>
		  </tr>
		  <tr>
