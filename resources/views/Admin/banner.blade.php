...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1">
  ...
  <tr>
    <td width="100%"><input type="button" value="新增" name="new" onclick="window.location='/banner_new'"></td>
  </tr>
      ...
      <tr>
        ...
        @if($count != 0 && $count != -1)
          <td width="5%" align="center">
            <font size="3">
              @if($k == 0)
                <a href="/banner_sort/{{$v->sort}}/down">{{'▼'}}</a>
              @elseif($k == $count)
                <a href="/banner_sort/{{$v->sort}}/up">{{'▲'}}</a>
              @else
                <a href="/banner_sort/{{$v->sort}}/up">{{'▲'}}</a>&nbsp;
                <a href="/banner_sort/{{$v->sort}}/down">{{'▼'}}</a>
              @endif    
        ...
        <input type="button" value="編輯" name="update" onclick="window.location='/banner_edit/{{$v->sno}}'">
        @if($v->status == 1)
          <input type="button" value="關閉" name="close" onclick="window.location='/banner_off/{{$v->sno}}'">
        @else
          <input type="button" value="開啟" name="open" onclick="window.location='/banner_open/{{$v->sno}}'">
        @endif
        <input type="button" value="刪除" name="delete" onclick="window.location='/banner_delete/{{$v->sno}}'">
      </tr>
      @endforeach
    </table>
